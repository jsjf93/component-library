...
function newImpl(CATALOG: string[], byCurrency: Pair[]) {
  const lookup = new Map(byCurrency.map((p) => [p.base, p]));
  return CATALOG.map((b) => lookup.get(b)).filter(
    (p): p is Pair => Boolean(p)
  );
}

function newImplLoop(CATALOG: string[], byCurrency: Pair[]) {
...
function runBench() {
  ...
  const startNewLoop = performance.now();
  for (let i = 0; i < ITERS; i++) {
    newImplLoop(CATALOG, byCurrency);
  }
  const endNewLoop = performance.now();

  console.log(`Old: ${(endOld - startOld).toFixed(2)}ms`);
  console.log(`New (Map with map): ${(endNew - startNew).toFixed(2)}ms`);
  console.log(`New (Map with for-of): ${(endNewLoop - startNewLoop).toFixed(2)}ms`);
}
