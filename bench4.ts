...
function newImpl2(CATALOG: string[], byCurrency: Pair[]) {
  const catSet = new Set(CATALOG);
  return byCurrency.filter((p) => catSet.has(p.base));
}
