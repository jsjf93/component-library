...
function AssetData({ pair }: { pair: Pair }) {
  ...
  const position =
    range > 0 ? ((snapshot.last - snapshot.low24) / range) * 100 : 0;
  const volume = compactNumbers
    ? formatCompact(snapshot.volume24)
    : snapshot.volume24.toFixed(2);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <Progress
          ...
          value={position}
          formatValue={() =>
            `${formatCurrency(snapshot.low24, quote)} – ${formatCurrency(snapshot.high24, quote)}`
          }
        />
        <dl className="mt-6 grid grid-cols-2 gap-5 sm:grid-cols-4">
          <Stat
            label="24h high"
            value={formatCurrency(snapshot.high24, quote)}
          />
          <Stat label="24h low" value={formatCurrency(snapshot.low24, quote)} />
          <Stat
            label="Today's open"
            value={formatCurrency(snapshot.open, quote)}
          />
...
function AssetResolver({ symbol }: { symbol: string }) {
  ...
  return (
    <ErrorBoundary
      label={`${pair.wsSymbol} data`}
      onReset={() => resetAsset(pair.restName)}
    >
